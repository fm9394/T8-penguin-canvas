...
[`Canvas.tsx`](file:///e:/PenguinPravite/T8-penguin-canvas/src/components/Canvas.tsx) 全局 keydown 里新增 Ctrl+G 分支，选中节点后一键创建 `groupBox` 容器：
...
- **必须 `preventDefault()`**：浏览器默认 Ctrl+G = 「查找下一个匹配项」，不拦截会出浏览器查找条
- **加 `!e.shiftKey` 限定**：避免与未来可能的 Ctrl+Shift+G（拆组）冲突
- **过滤 `type !== 'groupBox'`**：不允许嵌套组，与右键菜单 disabled 逻辑保持一致
- **useEffect deps 补 `handleCreateGroup, nodes`**：避免闭包捕获陈旧的选中集合

### 25.3 右键菜单与帮助文档同步

- 右键菜单「打组 (选中后)」→ 「打组 (Ctrl+G)」，让用户能看到快捷键
- [`CanvasToolbar.tsx`](file:///e:/PenguinPravite/T8-penguin-canvas/src/components/CanvasToolbar.tsx) 帮助面板新增 `['Ctrl + G', '选中多个节点后快捷打组']`

**原则**：任何可以双通道触发的操作（菜单 + 快捷键），菜单文案都要显示快捷键提示。

---

## 26. OutputNode 中继化与“中继展示型节点”仪表盘

### 26.1 背景

OutputNode 原本作为「终端预览节点」（只有 target handle，不向下游输出），但用户希望“预览、可编辑、同时可继续输出”。从「终端」升级为「中继展示型」。

### 26.2 改造点

- **[`portTypes.ts`](file:///e:/PenguinPravite/T8-penguin-canvas/src/config/portTypes.ts)**：`output.outputs` 从 `[]` 改为 `['any']`（与 relay 对齐）
- **[`OutputNode.tsx`](file:///e:/PenguinPravite/T8-penguin-canvas/src/components/nodes/OutputNode.tsx)**：
  - 右侧新增 source Handle (any)
  - 新增下游透传 useEffect，将 collected + displayText 写到自身 data

### 26.3 下游透传字段映射

| 自身 data 字段 | 写入内容 | 供下游节点读取 |
|---|---|---|
| `prompt` / `text` / `reply` | `displayText`（含用户编辑覆盖） | 文本下游 |
| `imageUrl` | `collected.images[0]` | 单图下游 |
| `imageUrls` / `urls` | `collected.images.slice()` | 多图下游 |
| `videoUrl` | `collected.videos[0]` | 视频下游 |
| `audioUrl` | `collected.audios[0]` | 音频下游 |

### 26.4 防循环关键措施

- **不写 `outputText` 字段**：保留 `outputText` 的「用户编辑覆盖标记」语义，避免与 displayText 计算依赖互锐
- **update 前手式比较 cur vs next**，一致就跳过，不触发 setNode
- **useEffect deps `[displayText, collected]` 不含 `d`**：自身 data 变化不会回灌触发循环

### 26.5 中继展示型节点仪表盘（新增同类节点须遵守）

从节点衔接的「中继展示」位置看，需同时满足：

1. **上游订阅机制**：`useNodeConnections({ id, handleType: 'target' }) + useNodesData(upstreamIds)`（参考第 23 章）
2. **下游透传**：`useEffect([收集结果])` + 手式 cur/next 比较 + `update()` 写不同于订阅依赖的字段
3. **两个 Handle 都显式定位**：`top: 50%, left/right: -5, transform: translateY(-50%), minWidth/minHeight: 10`
4. **容器禁用 `overflow: hidden`**：会裁掉伸出边缘 5px 的 Handle 圆点（参见 26.7）
5. **portTypes 设 `outputs: ['any']`**：与任意下游兼容

### 26.6 中继型节点 vs 终端型节点 vs 中继展示型节点对比

| 类型 | 代表节点 | 上游订阅 | 下游透传 | 是否调 `update()` |
|---|---|---|---|---|
| 中继型 | RelayNode | `useReactFlow.getEdges/getNodes` + `useMemo upstreamSignature` | useEffect([sig]) | 是 |
| 终端型 | StoryboardGridNode（旧型） | `useNodeConnections + useNodesData` | 无 | 否 |
| **中继展示型** | **OutputNode（新）** | `useNodeConnections + useNodesData` | `useEffect([displayText, collected])` + 手式比较 | **是**（但避免踩订阅依赖字段）|

### 26.7 Handle 被容器 overflow:hidden 裁掉的踩坑

**现象**：节点两侧 Handle 圆点只看到一半，被节点边框裁掉。

**原因**：xyflow Handle 默认位置会伸出节点边缘（这里用 `left/right: -5` 让它伸出 5px）。如果节点容器加了 `overflow: hidden`，伸出部分全部被裁。

**踩坑调动**：这个 `overflow: hidden` 是为了第 24 章「节点外白色矩形」兑底加的，但那个 bug 的根因已在全局 CSS 彻底修了，overflow 兑底不再需要。**踩坑需要反复复盘：创口性修复后要记得去掉老的兑底层**，否则会连环出新 bug。

**规范**：凡节点容器要加 `overflow: hidden`，必须事先确认所有 Handle 不伸出边缘（或者 Handle 用 `inset 0` 定位贴在边上不伸出）。

---

## 27. 批量 / 组执行调度逻辑与依赖传递修复
...
| 层 | 职责 | 实现位置 |
|---|---|---|
| 1. 节点 runFn 完成回报 | 节点 generate 完成后才 markDone | [`useRunTrigger`](file:///e:/PenguinPravite/T8-penguin-canvas/src/hooks/useRunTrigger.ts) |
| 2. 调度串行化 | 逐个 await lastDone, 上一个完成才启下一个 | [`runNodesByOrder`](file:///e:/PenguinPravite/T8-penguin-canvas/src/components/Canvas.tsx) |
| 3. 拓扑排序 | Kahn 算法 + 跨中继传递依赖 | [`topologicalSort`](file:///e:/PenguinPravite/T8-penguin-canvas/src/utils/topologicalSort.ts) |
| 4. 跳节点 data 同步 | 下游读上游 data 是最新值 | xyflow setNodes + React state |

### 27.2 第 1 层: useRunTrigger 的 markDone 时机
...
useEffect(() => {
  ...
  (async () => {
    try {
      await runFnRef.current(); // 等 handleGenerate 整个 Promise resolve
      markDone(nodeId, true);
    } catch (e) {
      markDone(nodeId, false, e?.message);
    }
  })();
}, [target, nodeId, markDone]);
```

**关键保障**：业务节点的 `handleGenerate` 必须是「submit + 轮询直到 SUCCESS + 写入 data」的完整 async，不能「投递完就 resolve」，否则 markDone 的时机不准。现有 LLMNode / VideoNode / SeedanceNode / AudioNode / RunningHubNode 都遵守这个约定。

### 27.3 第 2 层: runNodesByOrder 串行调度
...
for (let i = 0; i < order.length; i++) {
  await new Promise<void>((resolve) => {
    ...
    const timer = window.setTimeout(finish, 5 * 60 * 1000); // 5分钟超时兑底
    triggerRun(id, 'batch');
  });
}
```

严格串行 await。同一拓扑层多节点也串行，换可靠性 > 吞吐。

### 27.4 第 3 层: topologicalSort 跨中继传递依赖（本次修复重点）

**旧逻辑的 bug**：`topologicalSort` 只保留「两端都是 executable 节点」的边作为依赖，中继节点（`output` / `relay` / `upload` / `idea` / `bp`）会使依赖边被忽略，上下游 executable 节点变成「入度都为 0」同时入队，顺序不可控。

**示例重现**：组里有链路 `llm-A → output(中继) → llm-B`：
- `output` 不在 EXECUTABLE_NODE_TYPES
- 边 `llm-A→output` 、`output→llm-B` 两端不同时 executable → 被忽略
- `llm-A` `llm-B` 入度都为 0 → nodes 数组顺序决定入队顺序 → **可能 llm-B 先跑**，这时 llm-A 还没出图，llm-B 拿到空数据

**修复**：在 [`topologicalSort`](file:///e:/PenguinPravite/T8-penguin-canvas/src/utils/topologicalSort.ts) 内对每个 executable 节点 BFS，跨越非 executable 中继节点建立「传递依赖」：

```ts
for (const src of exeIds) {
  const stack = [...(fullAdj.get(src) || [])];
  ...
  while (stack.length) {
    ...
    visited.add(cur);
    if (exeIds.has(cur)) {
      // 遇到下一个 executable: 建立 src→cur 传递依赖, 不再穿越它
      adj.get(src)!.push(cur);
      inDegree.set(cur, (inDegree.get(cur) || 0) + 1);
    } else {
      // 非 executable 中继, 继续向下扩展不计依赖
...
修复后 `llm-A → output → llm-B` 会建立隐式依赖 `llm-A → llm-B`，llm-B 必须等 llm-A 完成才启动。

### 27.5 第 4 层: 跳节点 data 同步

因为串行上 `await`，上个节点 markDone 时 update 已经写入 React state。下个节点 trigger 时：
- 用 `useNodeConnections + useNodesData` 的节点 → 会同步拿到上游最新 data
- 用 `useReactFlow().getNodes()` 的节点 → 同步拿到 store 最新快照
- 用 props.data 的节点 → React 重渲染后会联动拿到最新值

三者都保证上游 data 是「已生成完」的状态。

### 27.6 EXECUTABLE_NODE_TYPES 与「中继节点」对比

| executable（在调度顺序里） | 中继 / 辅助（不入座但会传递依赖） |
|---|---|
| `image` / `edit` / `multi-angle-3d` / `panorama-720` / `penguin-portrait` | `output` |
| `video` / `seedance` / `audio` / `llm` / `runninghub` | `relay` |
| `resize` / `upscale` / `grid-crop` / `remove-bg` / `combine` / `frame-extractor` | `upload` / `idea` / `bp` / `groupBox` |

**拓展原则**：新增业务节点时，如果是「需要调 generation API 生成」类型，务必加进 EXECUTABLE_NODE_TYPES；如果是「只传递 / 预览 / 资源源」类型，不加进拓扑就会自动靠 27.4 的 BFS 传递。
...
```
upload(图) → llm-A(图生图) → output(中继) → llm-B(再编辑)
```

全选 → Ctrl+G 打组 → 点右上角 ▶。期望行为：
- llm-A 出现 polling/submitting 状态，其他节点不动
- llm-A 出图后，output 立即显示，同时透传给 llm-B
- llm-B 才进入 polling/submitting

如果看到 llm-A 和 llm-B 同时启动，则传递依赖失效，要检查 27.4 的 BFS 逻辑是否在生效。

### 27.8 关键 Commit 索引

- `ba5d754` 修复 topologicalSort 跨中继传递依赖（本章重点）
- `3cc7cd0` OutputNode 升级为中继展示型节点
- `b60c329` 去掉 OutputNode `overflow: hidden` 修复 Handle 被裁
- `04728c8` Ctrl+G 快捷键打组
